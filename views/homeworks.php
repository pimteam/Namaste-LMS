...
<div class="wrap">
	...
	<p><a href="admin.php?page=namaste_homeworks&course_id=<?php echo (int)$_GET['course_id']?>&lesson_id=<?php echo (int)$_GET['lesson_id']?>&do=add"><?php _e('Click here to create new assignment', 'namaste')?></a></p>
		<?php if(sizeof($homeworks)):?>
			<table class="widefat">
				...
				<?php foreach($homeworks as $homework):
					...
					<tr class="<?php echo $class?>"><td><?php echo stripslashes($homework->title)?></td><td><a href="admin.php?page=namaste_homeworks&course_id=<?php echo (int)$_GET['course_id']?>&lesson_id=<?php echo (int)$_GET['lesson_id']?>&do=edit&id=<?php echo $homework->id?>"><?php _e('Edit', 'namaste')?></a></td>
					<td>
					<?php
			...
			<p><?php _e('Shortcode to publish these assignments on the front-end:', 'namaste')?> <input type="text" readonly="readonly" onclick="this.select()" value='[namaste-assignments lesson_id="<?php echo (int)$_GET['lesson_id']?>"]' size="50"><br />
